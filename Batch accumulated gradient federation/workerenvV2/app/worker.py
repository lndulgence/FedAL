...
def round(model, roundnumber,  epochs, bsize, db, lr, IPAddr, train, val, r_limit):
    ...
    rpe=math.ceil(len(train)/bsize)
    roundmodule=roundnumber%rpe
    # Callbacks
    start=roundmodule*bsize
    end=start+bsize
    ...
    if end<len(train):
        ...
        for i in train[start:-1]:
            x_train.append(i[0])
            y_train.append([float(i[1])])
    x_train= np.array(x_train)
    y_train= np.array(y_train)  
    grads=step(x_train, y_train, model)
    weights=dill.dumps(grads)
